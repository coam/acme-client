...
//! AcmeOrderAuthorizationChallengeList object for a domain name and fulfill at least one challenge (http or dns for
//! Let's Encrypt).
//!
//! To create an AcmeOrderAuthorizationChallengeList object for a domain:
//!
//! ```rust,no_run
//! # use acme_client::libs::error::Result;
//! # fn try_main() -> Result<()> {
...
//! ```
//!
//! [AcmeOrderAuthorizationChallengeList](struct.AcmeOrderAuthorizationChallengeList.html) object will contain challenges created by
//! ACME server. You can create as many AcmeOrderAuthorizationChallengeList object as you want to verify ownership
...
//!
//! `acme-client` has
//! [`save_auth_key_token`](struct.AcmeOrderAuthorizationChallenge.html#method.save_auth_key_token) method
...
//! [`signature`](struct.AcmeOrderAuthorizationChallenge.html#method.signature) method.
//!
...
fn load_response(response: &mut reqwest::blocking::Response) -> Result<String> {
    ...
    trace!("[ACME-API-RESPONSE-INFO]: \n{:?}", response);
    trace!("[ACME-API-RESPONSE-CONTENT]: \n{}", content);
...
impl AcmeAuthDirectory {
    ...
    pub fn from_url(url: &str) -> Result<AcmeAuthDirectory> {
        ...
        Ok(AcmeAuthDirectory {
            ...
            auth_directory: from_str(&response_content)?,
        })
    }
    ...
    pub fn get_nonce(&self) -> Result<String> {
        ...
        trace!("[获取临时授权凭证->get_nonce()][acme_resource_api: {}]", acme_resource_api);

        // 请求ACME服务 - 获取接口临时调用凭证...
        let client = Client::new();
        let mut response = client.get(acme_resource_api).send()?;
    ...
    fn request<T: Serialize>(&self, private_key: &PKey<openssl::pkey::Private>, request_acme_resource: &str, payload: T, kid: Option<String>) -> Result<(StatusCode, HeaderMap, Value)> {
        ...
        let acme_resource_api = {
            ...
            match request_acme_resource {
                // Directory...
                "keyChange" | "newAccount" | "newNonce" | "newOrder" | "revokeCert" => self
                    .get_acme_resource_url(request_acme_resource)
                    .ok_or(format!("URL for request_acme_resource: {} not found", request_acme_resource))?,
                // API 接口
                _ => {
                    // 仅支持挑战验证...
                    ...
                    if !request_acme_resource.starts_with(acme_challenge_api.as_str()) && !request_acme_resource.starts_with(acme_finalize_api.as_str()) {
                        panic!(
                            "[不支持此接口类型][request_acme_resource: {}][acme_challenge_api: {}][acme_finalize_api: {}]",
                            request_acme_resource, acme_challenge_api, acme_finalize_api
                        )
                    }
    ...
    pub fn jws<T: Serialize>(&self, request_acme_resource_api: &str, private_key: &PKey<openssl::pkey::Private>, payload: T, kid: Option<String>) -> Result<HashMap<String, Value>> {
        ...
        if let Some(k_id) = kid {
            ...
            //protected.insert("jwk".to_owned(), from_str(self.jwk(private_key)?.as_str()));
        ...
        trace!("[接口签名结果->request()->jws()][signature: {}]", signature);

        request_jws.insert("signature".to_owned(), to_value(signature)?);
    ...
    pub fn jwk(&self, private_key: &PKey<openssl::pkey::Private>) -> Result<Value> {
        ...
        //jwk.insert("kty".to_owned(), "RSA".to_owned());
        //jwk.insert("n".to_owned(), b64(&rsa.n().to_vec()));
        //info!("[JWK][HashMap][jwk: {:?}][jwk.keys(): {:?}]", jwk, jwk.keys());
...
pub struct AcmeOrderAuthData {
    // 订单身份凭证
    pub order_auth_identifier: AcmeOrderIdentifier,
    // DNS-01 授权验证挑战
    pub order_auth_challenge: AcmeOrderAuthorizationChallenge,
}
...
impl AcmeAccountData {
    ...
    pub fn acme_order_creator(&self) -> AcmeOrderCreator {
        AcmeOrderCreator { acme_order_identifiers: None }
    }
}
...
impl AcmeAccountRegistration {
    ...
    pub fn register(self) -> Result<AcmeAccountData> {
        ...
        //debug!("[账户注册结果][response_data: {}]", response_data);

        match status {
            StatusCode::OK => {
                info!("[账户注册成功] -> StatusCode::OK - User successfully registered!")
            }
            StatusCode::CREATED => {
                info!("[账户注册成功] -> StatusCode::CREATED - User successfully registered!")
            }
            StatusCode::CONFLICT => {
                info!("[账户注册成功] -> StatusCode::CONFLICT - User already registered!")
            }
            _ => return Err(ErrorKind::AcmeServerError(response_data).into()),
        };
...
pub struct AcmeOrderData {
    ...
    // 获取订单授权挑战
    pub order_auth_list: Vec<AcmeOrderAuthResponse>,
}
...
impl AcmeOrderData {
    ...
    pub fn request_acme_order_auth_list(&mut self) -> Result<&AcmeOrderData> {
        ...
        for authorization in &self.authorizations {
            ...
            let acme_order_auth_response: AcmeOrderAuthResponse = from_str(&response_content)?;
            ...
            // 过滤查询 dns-01 挑战...
            let order_auth_challenge = order_auth_challenges.iter().find(|challenge| challenge.types == "dns-01");

            // 必须包含 `dns-01` 验证请求
            if let None = order_auth_challenge {
                panic!("[未匹配挑战验证: dns-01,授权验证中断!] -- Find none [dns-01] in challenges");
            }

            // 推入授权列表...
            self.order_auth_list.push(acme_order_auth_response);
        }
        println!("\n");

        debug!("[ACME 订单授权验证列表] ->[acme_order.order_auth_list: {:?}]", self.order_auth_list);
    ...
    pub fn get_acme_order_auth_list(&self, acme_account: &AcmeAccountData) -> Result<Vec<AcmeOrderAuthData>> {
        info!("[循环计算 ACME 订单授权签名] ->[order_auth_list: {:?}]", self.order_auth_list);

        // 授权列表...
        let mut acme_order_auth_list = Vec::<AcmeOrderAuthData>::new();

        // 循环授权挑战验证列表...
        for order_auth_info in &self.order_auth_list {
            println!("\n");
            info!("[为 ACME 订单配置挑战签名] -> [auth_name: {}][order_auth_info: {:?}]", order_auth_info.identifier.value, order_auth_info);

            // 依次读取授权数据
            let order_auth_identifier = order_auth_info.identifier.clone();
            let order_auth_challenges = order_auth_info.challenges.clone();
            debug!("[####] order_auth_identifier: {:?}", order_auth_identifier);
            debug!("[####] order_auth_challenges: {:?}", order_auth_challenges);

            // 获取 [DNS-01] 授权挑战...
            let mut order_auth_challenge = order_auth_challenges.iter().find(|challenge| challenge.types == "dns-01").unwrap().clone();

            debug!("[####] order_auth_challenge: {:?}", order_auth_challenge);

            // 判断是否已经过验证
            if order_auth_challenge.status == "valid" {
                info!("该挑战已为验证状态,取消重复签名验证!");
                continue;
            }
            ...
            // 比对测试数据...
            //let token = String::from("lKV1pdAuaKJRYcpp-0WzSJsl2IZvomeOfkqrshlkamo");

            // 获取 jwk ...
            let jwk = &acme_account.directory().jwk(acme_account.private_key())?;
            ...
            let auth_key_thumbprint = b64(&hash(MessageDigest::sha256(), &to_string(jwk)?.into_bytes())?);
            //info!("[JWK][jwk: {:?}][auth_key_thumbprint: {:?}]", jwk, auth_key_thumbprint);

            // 授权 Token ...
            let auth_key_token = format!("{}.{}", token, auth_key_thumbprint);

            info!("[DNS签名][auth_key_thumbprint: {:?}][auth_key_token: {:?}]", auth_key_thumbprint, auth_key_token);

            // 配置授权 Key ...
            order_auth_challenge.auth_key_token = Some(auth_key_token);

            // 获取挑战签名...
            let auth_dns_token = order_auth_challenge.signature().unwrap();

            // 保存授权...
            order_auth_challenge.auth_dns_token = Some(auth_dns_token.clone());

            info!("[DNS挑战签名][auth_dns_token: {:?}]", auth_dns_token);

            // 授权挑战数据...
            let acme_order_auth_data = AcmeOrderAuthData {
                order_auth_identifier,
                order_auth_challenge,
            };

            // 推入授权列表...
            acme_order_auth_list.push(acme_order_auth_data);
        }

        println!("\n");

        debug!("[ACME 待验证订单授权签名列表] -> [acme_order_auth_list: {:?}]", acme_order_auth_list);
        println!("\n");
    ...
    pub fn verify_acme_order_auth_list(&self, acme_name: String, acme_account: &AcmeAccountData, acme_order_auth_list: &Vec<AcmeOrderAuthData>) -> Result<String> {
        ...
        for auth_acme_order_data in acme_order_auth_list.iter() {
            println!("\n");
            debug!("[###]循环处理挑战订单: auth_acme_order_data: {:?}", auth_acme_order_data);
            info!(
                "[###]验证挑战: [order_auth_challenge.url: {:?}][order_auth_challenge.auth_dns_token: {:?}]",
                auth_acme_order_data.order_auth_challenge.url, auth_acme_order_data.order_auth_challenge.auth_dns_token
            );

            // 依次读取授权数据
            let order_auth_identifier = auth_acme_order_data.order_auth_identifier.clone();
            let order_auth_challenge = auth_acme_order_data.order_auth_challenge.clone();
            debug!("[####] order_auth_identifier: {:?}", order_auth_identifier);
            debug!("[####] order_auth_challenge: {:?}", order_auth_challenge);

            // 判断是否已经过验证
            if order_auth_challenge.status == "valid" {
                info!("该挑战已为验证状态,取消重复发起挑战验证请求!");
                continue;
            }
            ...
            // 发起授权验证...
            let dns_challenge_results = self.validate(acme_account, order_auth_challenge);

            // 如果验证成功,则跳出验证请求...
            if let Ok(()) = dns_challenge_results {
                warn!("[验证完成,跳出循环验证请求][acme_name: {}]", acme_name);
                //warn!("[验证完成,跳出循环验证请求][acme_name: {}][try_ts: {}]", acme_name, try_ts);
                //break 'outer;
            } else {
                error!("[验证超时失败][acme_name: {}]", acme_name);
                //error!("[验证超时失败][acme_name: {}][try_ts: {}]", acme_name, try_ts);
                //return Err(ErrorKind::AcmeServerError(from_str("验证失败中断!")?).into());
            }
    ...
    /// Triggers validation.
    pub fn validate(&self, acme_account: &AcmeAccountData, challenge: AcmeOrderAuthorizationChallenge) -> Result<()> {
        ...
        let (status, _response_headers, response_data) = acme_account
            .directory()
            .request(&private_key, challenge.url.as_str(), payload, Some(acme_account.account_url.clone()))?;
    ...
    pub fn finalize_order(&mut self, acme_account: &AcmeAccountData, acme_certificate_signer: &AcmeCertificateSigner) -> Result<&AcmeOrderData> {
        ...
        let private_key = acme_account.private_key();
        let (_status, _response_headers, response_data) = acme_account
            .directory()
            .request(&private_key, finalize_url.as_str(), payload, Some(acme_account.account_url.clone()))?;
        ...
        if let None = acme_order_response.certificate {
            panic!("异常订单:未核发证书签发地址!")
        }

        info!("[#][ACME订单核发的证书签发地址][acme_order_response.certificate: {:?}]", acme_order_response.certificate);
    ...
    pub fn sign_certificate(&self, acme_certificate_signer: AcmeCertificateSigner) -> Result<AcmeSignedCertificate> {
        ...
        debug!("[获取已签发订单域名证书][certificate_url: {:?}]", certificate_url);
        ...
        if response.status() != StatusCode::OK {
            // 处理 ACME 响应数据...
            let response_content = load_response(&mut response)?;
            return Err(ErrorKind::AcmeServerError(from_str(&response_content)?).into());
        }
        ...
        // 打印证书...
        let cert_data = String::from_utf8_lossy(&crt_der);
        info!("[###]签发域名证书: \n{}", cert_data);
        ...
        Ok(AcmeSignedCertificate {
            cert,
            csr: acme_certificate_signer.csr.unwrap(),
            private_key: acme_certificate_signer.private_key.unwrap(),
        })
    }
}
...
impl AcmeOrderCreator {
    ...
    pub fn create(self, account: &AcmeAccountData) -> Result<AcmeOrderData> {
        ...
        // 创建订单域名...
        order_params.insert("identifiers".to_owned(), &self.acme_order_identifiers);

        info!("[创建订单请求参数][resources: createOrder][order_params: {:?}]", order_params);

        // ACME 账户配置...
        let private_key = &account.private_key;
        let account_url = &account.account_url;

        let (status, response_headers, response_data) = account.directory().request(&private_key, "newOrder", order_params, Some(account_url.clone()))?;

        debug!("[订单创建结果][response_data: {}]", response_data);
...
//pub struct AcmeOrderAuthorizationChallengeList(pub Vec<AcmeOrderAuthorizationChallenge>);
//
//impl AcmeOrderAuthorizationChallengeList {
//    /// Gets a challenge.
//    ///
//    /// Pattern is used in `starts_with` for type comparison.
//    pub fn get_challenge(&self, pattern: &str) -> Option<&AcmeOrderAuthorizationChallenge> {
...
//
//    /// Gets http challenge
//    pub fn get_http_challenge(&self) -> Option<&AcmeOrderAuthorizationChallenge> {
//        self.get_challenge("http")
//    }
//
//    /// Gets dns challenge
//    pub fn get_dns_challenge(&self) -> Option<&AcmeOrderAuthorizationChallenge> {
//        self.get_challenge("dns")
//    }
//
//    /// Gets tls-sni challenge
//    pub fn get_tls_sni_challenge(&self) -> Option<&AcmeOrderAuthorizationChallenge> {
//        self.get_challenge("tls-sni")
//    }
//
//    /// Gets all dns challenge
//    pub fn get_dns_challenges(&self) -> Option<&Vec<AcmeOrderAuthorizationChallenge>> {
//        Some(&self.0)
//    }
//}

// AcmeOrderAuthorizationChallenge 挑战 - A verification challenge...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AcmeOrderAuthorizationChallenge {
    ...
    pub url: String,
    /// AcmeOrderAuthorizationChallenge token.
    pub token: String,
...
// AcmeOrderAuthorizationChallenge 挑战...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AcmeOrderAuthorizationChallengeValidationRecord {
    hostname: String,
}

impl AcmeOrderAuthorizationChallenge {
    ...
    pub fn signature(&self) -> Result<String> {
        Ok(b64(&hash(MessageDigest::sha256(), &self.auth_key_token().into_bytes())?))
    }
    ...
    pub fn auth_key_token(&self) -> String {
        self.auth_key_token.clone().unwrap()
    }
}
...
impl AcmeSignedCertificate {
    ...
    fn get_intermediate_certificate(&self, url: Option<&str>) -> Result<X509> {
        // 请求ACME服务 - 获取中间证书链...
        let client = Client::new();
        let mut response = client.get(url.unwrap_or(LETS_ENCRYPT_INTERMEDIATE_CERT_URL)).send()?;
        let mut content = Vec::new();
