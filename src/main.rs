...
fn main() {
    let matches = App::new(env!("CARGO_PKG_NAME"))
        ...
        .subcommand(SubCommand::with_name("sign")
            ...
            .arg(Arg::with_name("DIRECTORY")
                ...
                .short("A")
                .long("directory")
                .default_value(acme_client::libs::v1::LETSENCRYPT_DIRECTORY_URL)
                .takes_value(true)
                .display_order(1))
    ...
    //pretty_env_logger::init_custom_env(env_logger::Env::default().default_filter_or("info").into());
...
//            let san_extension_idx = X509v3_get_ext_by_NID(extensions, openssl_sys::NID_subject_alt_name, -1);
//            if let Some(san_extension) = Stack::<X509Extension>::from_ptr(extensions).iter().nth(san_extension_idx as usize) {
//                let extension_data = X509_EXTENSION_get_data(san_extension.as_ptr());
//                let slc = slice::from_raw_parts((*extension_data).data, (*extension_data).length as usize);
...
//    let names_from_csr = names_from_csr("tests/data/domain_with_san.csr").unwrap();
